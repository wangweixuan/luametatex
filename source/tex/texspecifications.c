    ...
    while (1) {
        ...
        switch (tex_scan_character("aorvdlpifAORVDLPIF", 1, 1, 1)) {
            ...
            case 'r': case 'R':
                ...
                break;
            case 'v': case 'V':
                if ((valid & specification_option_values) && tex_scan_mandate_keyword("values", 1)) {
                    options |= specification_option_values;
                }
                break;
            case 'd': case 'D':
...
static halfword tex_aux_scan_specification_par_shape(void)
{
    ...
    if (count > 0) {
     // halfword options = tex_scan_partial_keyword("options") ? tex_scan_integer(0, NULL) : 0;
        ...
        for (int n = 1; n <= count; n++) {
            tex_set_specification_indent(spec, n, tex_scan_dimension(0, 0, 0, 0, NULL));
            tex_set_specification_width(spec, n, tex_scan_dimension(0, 0, 0, 0, NULL)); 
        }
        return spec; 
    } else {
        return null; 
...
static halfword tex_aux_scan_specification_fitness_demerits(void)
{
    halfword count = tex_scan_integer(1, NULL);
    halfword spec = null;
                        ...
                        switch (tex_scan_character("ntNT", 0, 0, 0)) {
                            ...
                            case 't': case 'T':
                                if (tex_scan_mandate_keyword("fitnessdemerits", 3)) {
                                    tex_set_passes_fitnessdemerits(p, n, tex_aux_scan_specification_fitness_demerits());
